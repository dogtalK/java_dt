...
SELECT OR_ME_ID AS 주문아이디, PR_NAME AS제품명, OP_NAME AS 옵션명, PR_PRICE AS 제품가격, OL_AMOUNT AS 구매수량
FROM `ORDER` 
	JOIN ORDER_LIST ON OL_OR_NUM = OR_NUM
    JOIN `OPTION` ON OL_OP_NUM = OP_NUM
    JOIN PRODUCT ON OP_PR_CODE = PR_CODE
    WHERE OR_STATE NOT IN('환불','조회') AND OR_ME_ID = 'qwe123';
    
-- abc123회원이 장바구니에 담은 제품명과 옵션명, 수량을 확인하는 쿼리
SELECT BA_ME_ID AS 회원아이디, PR_NAME AS제품명, OP_NAME AS 옵션명, BA_AMOUNT AS 수량
FROM BASKET
...
-- abc123회원이 작성한 리뷰드가 있는 제품명을 조회
SELECT PR_NAME AS 리뷰제품
FROM REVIEW
