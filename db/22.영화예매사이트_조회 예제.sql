...
-- abc회원이 예매 내역을 조회하는 쿼리(영화명, 시간, 좌석명)
SELECT 
...
-- abc회원이 예매 내역을 조회하는 쿼리(영화명, 시간, 성인 X명, 청소년 X명)
SELECT 
...
-- CGV강남 영화관에서 콘크리트 유토피아 20:50에 예매 가능한 좌석을 조회한는 쿼리(OUTER JOIN) 
SELECT 
	SE_NAME AS 좌석
FROM
	SEAT
